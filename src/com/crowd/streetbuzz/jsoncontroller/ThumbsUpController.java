 ...
 *
 */
public class ThumbsUpController implements Controller, Constants{
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		uaone.setActiondetails("Voted up card id "+cardid+" from user "+userid);
		uaone.setActiontype(ACTION_VOTEUP);
		uaone.setEntityid(cardidLong);
		uaone.setEntityuserid(entityuserid);
		uaone.setPoints(new Long(ACTION_VOTEUP_VALUE));
		uaone.setUserid(useridLong);
		
		UserActivities uatwo = new UserActivities();
		uatwo.setActionarea("");
		uatwo.setActiondatetime(new Date());
		uatwo.setActiondetails("Got card id "+cardid+" voted up by user "+userid);
		uatwo.setActiontype(ACTION_GET_VOTE_UP);
		uatwo.setEntityid(cardidLong);
		uatwo.setEntityuserid(entityuserid);
		uatwo.setPoints(new Long(ACTION_GET_VOTE_UP_VALUE));
