 ...
 * 
 */
public class TaxonomyTree implements Constants, SystemConstants {
	private static final String seedWordsList =
	"Business,Technology,Culture,Entertainment,Education,Work,Family,Living,Fashion,Beauty,Food,Drinks,Gadgets,Appliances,Health,Fitness,Home,Garden,Motors,Vehicles,Nature,Climate,Society,Politics,Economy,Religion,Philosophy,Sports,Hobbies,Travel,Tourism,Science,Medicine";
	//private static final String seedWordsList = "Fitness,Vehicles";
	...
	public static void seed(TaxonomyDAO taxonomyDAO) {
		String[] seedArray = seedWordsList.split(",");
		for (int i = 0; i < seedArray.length; i++) {
			String searchterm = seedArray[i];
			searchterm = searchterm.trim();
			Map finalMap = runSearch(searchterm);
			// System.out.println("finalList.size==> " + finalList.size());
			process(finalMap, searchterm, taxonomyDAO);
		}
	}

	private static void process(Map finalMap, String searchterm,
			...
			if (!compare.equalsIgnoreCase(searchterm)) {
				...
				if (taxonomy != null) {
					...
					if (processbool) {
						...
						taxonomynew.setCategoryid(getCategoryId(searchterm));
						taxonomynew.setType("phrase");
						taxonomynew.setRun(new Integer(1));
				...
				if (!compare.equalsIgnoreCase(searchterm)) {
					...
					if (taxonomy != null) {
						...
						if (processbool) {
							...
							taxonomynew.setScore(new Integer(1));
							taxonomynew
									.setCategoryid(getCategoryId(searchterm));
	...
	private static Map runSearch(String searchterm) {
		...
		if (googlebool) {
			...
			if (count > 10) {
				grList = googleResultList.subList(0, 8);
			}
		}
		...
		List linksList = extractLinks(googleResultList, farooSearchResultList,
